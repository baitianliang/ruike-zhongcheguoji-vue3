...
async function persistSnapshot({ manual = false } = {}) {
  ...
  try {
    ...
    status.value = manual ? '已保存共享基线' : '已保存共享基线'
    if (showStructure.value) {
      workbookStructure.value = buildWorkbookStructure(snapshot)
    }
  } catch (error) {
    console.error(error)
    status.value = '保存共享基线失败'
  }
}
...
async function getInitialWorkbookData() {
  try {
    ...
    if (sharedWorkbook?.snapshot) {
      ...
      status.value = '已从服务端恢复共享表格'
      return sharedWorkbook.snapshot
    }
  } catch (error) {
    console.error(error)
    status.value = '读取共享表格失败，使用示例数据'
  }
...
async function applyRemoteCommand(entry) {
  ...
  try {
    ...
    status.value = '已合并远端操作'
    if (showStructure.value) {
      refreshWorkbookStructure()
    }
  } catch (error) {
    console.error(error)
    status.value = '合并远端操作失败'
  } finally {
...
async function openFindDialog() {
  ...
  try {
    ...
    console.error(error)
    status.value = '打开查找对话框失败'
  }
}

async function openReplaceDialog() {
  if (!univerAPIInstance) {
    status.value = '替换功能暂不可用'
    return
  }
  ...
  try {
    ...
    console.error(error)
    status.value = '打开替换对话框失败'
  }
}

async function openSortDialog() {
  if (!univerAPIInstance) {
    status.value = '排序功能暂不可用'
    return
  }
  ...
  <main class="sheet-app">
    <header class="app-bar">
      <div class="brand">
        ...
        <div>
          <h1>在线表格</h1>
          <p>{{ status }}<span v-if="lastSavedAt"> · {{ lastSavedAt }}</span></p>
        </div>
      </div>

      <nav class="actions" aria-label="表格操作">
        <button type="button" @click="saveDraft">立即同步</button>
        <button type="button" @click="loadDraft">恢复共享版本</button>
        <button v-if="false" type="button" @click="exportJson">导出 JSON</button>
      </nav>
    </header>

    <section class="sheet-shell" aria-label="Univer 在线表格">
      <div ref="container" class="univer-container"></div>

      <aside v-if="showStructure" class="structure-panel" aria-label="工作簿结构">
        <div class="structure-header">
          <div>
            <h2>工作簿结构</h2>
            <p v-if="workbookStructure">
              {{ workbookStructure.sheetCount }} 个工作表 · revision {{ workbookStructure.revision }}
            </p>
          </div>
          <button type="button" @click="refreshWorkbookStructure">刷新</button>
        </div>
        ...
        <div v-if="workbookStructure" class="sheet-list">
          ...
          >
            <div class="sheet-item-title">
              ...
              <span v-if="sheet.hidden">隐藏</span>
            </div>
            <p>{{ sheet.rowCount }} 行 × {{ sheet.columnCount }} 列</p>
            <p>{{ sheet.cellCount }} 个非空单元格 · {{ sheet.mergeCount }} 个合并区域</p>
            <code>{{ sheet.id }}</code>
          </article>
        </div>
      </aside>
